...
public class ForumDashboardController implements Initializable {
    ...
    @FXML
    private TableColumn<Publication, String> colAfficher;
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        Callback<TableColumn<Publication, String>, TableCell<Publication, String>> cellFactory3 = (param) -> {
            final TableCell<Publication, String> cell = new TableCell<Publication, String>() {
                @Override
                public void updateItem(String item, boolean empty) {
                    super.updateItem(item, empty);
                    if (empty) {
                        ...
                        btnAfficher.setOnAction(event -> {
                            Publication p = tableForum.getItems().get(tableForum.getSelectionModel().getSelectedIndex());
                            String date = p.getDatePublication();
                            String auteur = "Hamza Ben Beya";
                            String titre = p.getTitrePublication();
                            String contenu = p.getContenu();
                            try {
                                FXMLLoader loader = new FXMLLoader(getClass().getResource("AfficherPublication.fxml"));
                                Parent root = loader.load();
                                AfficherPublicationController r = loader.getController();
                                r.SetAuteur(auteur);
        ...
        colAfficher.setCellFactory(cellFactory3);
